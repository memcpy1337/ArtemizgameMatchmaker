...
public interface IMatchPublisher
{
    ...
    Task NewMatchReady(string matchId, string userId);
    ...
    Task MatchCancelled(string matchId, string userId, MatchCancelEnum matchCancel);
    Task MatchEnd(string matchId, List<MatchPlayerResult> results);
    Task ConnectionDataUpdate(string matchId, string userId, string address, int port);
}
